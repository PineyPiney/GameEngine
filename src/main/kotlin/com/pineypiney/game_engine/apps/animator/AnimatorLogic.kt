...
class AnimatorLogic(override val gameEngine: WindowedGameEngineI<AnimatorLogic>, var o: GameObject?, var creator: () -> GameObject) : WindowGameLogic() {
    ...
    val animationSelector: AnimationSelector.AnimationSelectorComponent = AnimationSelector(null, Vec2(-0.6f, 0.8f), Vec2(0.5f, 0.15f), this::setAnimation).getComponent<AnimationSelector.AnimationSelectorComponent>()!!
    private val componentSelector: ComponentSelector.ComponentSelectorComponent = ComponentSelector(null, Vec2(0.1f, 0.8f), Vec2(0.5f, 0.15f), this::setComponent).getComponent<ComponentSelector.ComponentSelectorComponent>()!!
    private val animationTimeLine = object: MenuItem(){

        init {
            position = Vec3(-0.4f, -0.9f, 0f)
            scale = Vec3(0.8f, 0.2f, 0f)
        }
    ...
    private val saveButton = TextButton("Save", Vec2(0.45f, -0.9f), Vec2(0.2f, 0.2f)){
    ...
    override fun render(tickDelta: Double) {
        ...
        (o?.getComponent<AnimatedComponent>())?.let { animationTimeLine.getComponent<AnimationTimeLine>()!!.value = it.animationTime }
    ...
    private fun setAnimation(){
        animationTimeLine.getComponent<AnimationTimeLine>()!!.setAnimationLength(animationSelector.item?.animation?.length ?: 1f)
    }
        ...
        if(item != null){
            ...
            if(component != null) {
                if(componentEditor == null) {
                    componentEditor = ComponentEditor(item, component, Vec2(0.5f, -0.4f), Vec2(0.5f, 0.8f), this::updateKeyFrameField).apply { init() }
                    add(componentEditor)
                }
