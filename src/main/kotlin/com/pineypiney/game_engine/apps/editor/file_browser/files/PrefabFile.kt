...
class PrefabFile(parent: GameObject, file: File, browser: FileBrowser): FileComponent(parent, file, browser) {
	...
	override fun getIcon(center: Vec2, size: Int): Sprite {
		...
		prefab.init()
		val renderers = prefab.catchRenderingComponents().mapNotNull { it.getComponent<RenderedComponentI>() }
		if(renderers.isEmpty()) return super.getIcon(center, size)
		...
		val prefabSize = maxPos - minPos
		val scale = 2f / maxOf(prefabSize.x, prefabSize.y, prefabSize.z)
	...
	override fun onPrimary(window: WindowI, action: Int, mods: Byte, cursorPos: CursorPosition): Int {
		...
		if(action == 0 && Timer.time - fileSelect > .5){
			...
			browser.screen.editManager.addEdit(AddObjectEdit(obj, null, browser.screen))
			return INTERRUPT
		}
		return action
	}
