...
class ComponentAdder(parent: GameObject, browser: ComponentBrowser): DefaultInteractorComponent(parent, "CPA"), RenderedComponentI {
	...
	override fun render(renderer: RendererI, tickDelta: Double) {
		...
		for(o in descendants){
			...
			if(renderers.isEmpty()){
				preRenderers.forEach { it.preRender(tickDelta) }
			}
			else{
				val vis = renderers.any { it.visible }
				preRenderers.forEach{ if(vis || !it.whenVisible) it.preRender(tickDelta)}
