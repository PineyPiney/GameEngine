...
class SliderPointerComponent(parent: GameObject) : DefaultInteractorComponent(parent, "SLP") {
	...
	override fun checkHover(ray: Ray, screenPos: Vec2): Float {
		...
		return if(screenPos.isWithin(Vec2(parent.transformComponent.worldPosition) - (size * 0.5f), size)) (ray.rayOrigin - parent.transformComponent.worldPosition.z).length()
		else -1f
	}
