...
object ByteData {
	...
	inline fun <T> bytesToNum(
		bytes: ByteArray,
		offset: Int,
		length: Int,
		bigEndian: Boolean,
		crossinline converter: UByte.() -> T,
		crossinline shl: (T, Int) -> T,
		or: Iterable<Int>.((Int) -> T) -> T
	): T {
//			or.invoke()
	...
	fun <T : Number, V : QuatT<T>> string2QuatT(string: String, tSize: Int, string2t: (String) -> T, construct: (T, T, T, T) -> V, default: V, wLast: Boolean = true): V {
		...
		return if (string.length == tSize * 4 + 3) {
			if (wLast) {
				construct(
					string2t(string.substring(3 * tSize + 3, 4 * tSize + 3)), string2t(string.take(tSize)),
					string2t(string.substring(tSize + 1..2 * tSize)), string2t(string.substring(2 * tSize + 2, 3 * tSize + 2))
				)
			} else {
				construct(
					string2t(string.take(tSize)), string2t(string.substring(tSize + 1..2 * tSize)),
			...
			if (wLast) {
				construct(
					string2t(string.substring(3 * tSize, 4 * tSize)), string2t(string.take(tSize)),
					string2t(string.substring(tSize, 2 * tSize)), string2t(string.substring(2 * tSize, 3 * tSize))
				)
			} else {
				construct(
					string2t(string.take(tSize)), string2t(string.substring(tSize, 2 * tSize)),
