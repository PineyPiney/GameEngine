 ...
 */
fun Vec3.coerceIn(low: Vec3, high: Vec3): Vec3 = Vec3(
	x.coerceIn(min(low.x, high.x), max(low.x, high.x)),
	y.coerceIn(min(low.y, high.y), max(low.y, high.y)),
	z.coerceIn(min(low.z, high.z), max(low.z, high.z))
)
...
fun Vec2.eerp(next: Vec2, delta: Float, exponent: Int) =
	Vec2(x.eerp(next.x, delta, exponent), y.eerp(next.y, delta, exponent))
...
fun Vec3.erp(next: Vec3, delta: Float, func: Float.(Float, Float) -> Float) =
	Vec3(x.func(next.x, delta), y.func(next.y, delta), z.func(next.z, delta))
...
fun Mat4.setScale(index: Int, scale: Vec3, currentScale: Vec3 = getScale(), res: Mat4 = Mat4(this)): Mat4{
...
fun Vec3.Companion.fromString(
	s: String,
	trim: Boolean = false,
	parse: String.() -> Float = java.lang.Float::parseFloat
): Vec3 {
	val trimmed = if (trim) s.trim().removePrefix("Vec3{").removeSuffix("}") else s
	return trimmed.split(",").map { if (trim) it.trim() else it }
		.let { Vec3(it[0].parse(), it[1].parse(), it[2].parse()) }
}

fun Vec3.Companion.fromHexString(s: String) = fromString(s) { intValue(16).bitsAsFloat }

fun Vec4.Companion.fromString(
	s: String,
	trim: Boolean = false,
	parse: String.() -> Float = java.lang.Float::parseFloat
): Vec4 {
	...
	return trimmed.split(",")
		.let { Vec4(it[0].trim().parse(), it[1].trim().parse(), it[2].trim().parse(), it[3].trim().parse()) }
}
