...
open class GamePad(val id: Int, val inputs: Inputs) {
    ...
    val rightJoystick get() = Vec2(axesStates[GLFW.GLFW_GAMEPAD_AXIS_RIGHT_X], -axesStates[GLFW.GLFW_GAMEPAD_AXIS_RIGHT_Y])
    val dPad get() = Vec2(
    ...
    fun updateAxes(axis: Int, state: Float){
        ...
        else if(abs(oldValue) > deadzone.y && abs(state) < deadzone.y) inputs.onInput(axis.s, 0, ControlType.GAMEPAD_AXIS)
