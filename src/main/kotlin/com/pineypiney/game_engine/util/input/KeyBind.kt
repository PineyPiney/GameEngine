...
class KeyBind(val key: Short, val controlType: Inputs.ControlType = Inputs.ControlType.KEYBOARD, val mods: Byte = 0) {
    ...
    fun matchMods(mods: Byte): Boolean {
        return (this.shift ||       mods and 1 <= 0) &&
                ...
                (this.super_ ||     mods and 8 <= 0) &&
                (this.caps ||       mods and 16 <= 0) &&
                (this.num ||        mods and 32 <= 0)
    }

    override fun equals(other: Any?): Boolean {
        (other as? KeyBind)?.let {
    ...
    override fun toString(): String {
        ...
        string += when(controlType) {
            Inputs.ControlType.KEYBOARD -> keyNames[this.key] ?: "Unknown"
            Inputs.ControlType.MOUSE -> mouseKeyNames[this.key] ?: "Unknown"
            Inputs.ControlType.JOYSTICK -> joystickKeyNames[this.key] ?: "Unknown"
            Inputs.ControlType.GAMEPAD_BUTTON -> gamepadKeyNames[this.key] ?: "Unknown"
            Inputs.ControlType.GAMEPAD_AXIS -> gamepadAxisKeyNames[this.key] ?: "Unknown"
        }
        return string
    }
