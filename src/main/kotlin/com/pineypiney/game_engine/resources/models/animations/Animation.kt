...
class Animation(val name: String, val frames: Array<KeyFrame>): Copyable<Animation> {
    ...
    fun getState(animationTime: Double, erp: String): Array<State> {
        ...
        val previousStates = frames.filter { it.time < animationTime }.reversed().flatMap { it.states.toList() }.distinctBy { it.parentId }.associateWith { state -> frames.firstOrNull { it.states.contains(state) }?.time ?: 0f }
        val nextStates = frames.filter { it.time > animationTime }.flatMap { it.states.toList() }.distinctBy { it.parentId }.associateWith { state -> frames.firstOrNull { it.states.contains(state) }?.time ?: 0f }
    ...
    override fun copy() = Animation(name, frames.copy())
